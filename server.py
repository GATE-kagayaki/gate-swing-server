...
TASK_SA_EMAIL = os.environ.get("TASK_SA_EMAIL", "") # Cloud Tasks 実行サービスアカウント
...
# --- 以前の高品質な解析ロジック関数群をそのまま保持 ---
# calculate_angle_3points, get_horizontal_angle, process_landmarks, extract_mediapipe_data, generate_pro_quality_text
# (スペース節約のため、コード内では省略。実際はそのまま配置してください)
...
def process_landmarks(frames_data):
    ...
    sway = (d_top.get("nose", [0,0])[0] - d_addr.get("nose", [0,0])[0]) * 100
    knee_sway = d_top.get("l_knee", [0,0])[0] - d_addr.get("l_knee", [0,0])[0]
...
def generate_pro_quality_text(metrics):
    """プロ品質の診断コメントを生成 (前回と同じロジック)"""
    # ... (前回の generate_pro_quality_text の中身をそのまま貼り付け)
    c = {} 
    drills = []
    fitting = {}
    sway = metrics["sway"]
...
def handle_task():
    ...
    try:
        ...
        doc_ref.update({
            ...
            "raw_data": frames_data # 解析に使った骨格データも保存
        })
...
def serve_report(report_id):
    ...
    # templates/report.html が存在することを前提とします。
