...
# ==================================================
# Free plan（月1回）制限
# users/{user_id} に monthly_count / monthly_reset を持たせる
# ==================================================
FREE_MONTHLY_LIMIT = 1  # ←月1回
...
def can_use_free_plan(user_id: str) -> bool:
    """
    free ユーザーが今月まだ使えるか（カウントは増やさない）
    """
    now = datetime.now(timezone.utc)
    doc_ref = users_ref.document(user_id)
    doc = doc_ref.get()

    # 未登録ユーザーは free として作成（まだ未使用）
    if not doc.exists:
        doc_ref.set({
            "plan": "free",
            "ticket_remaining": 0,
            "plan_expire_at": None,
            "monthly_count": 0,
            "monthly_reset": _month_start_utc(now),
    ...
    # free 以外はこの関数の対象外（＝ここでは止めない）
    if plan != "free":
        return True

    monthly_count = int(data.get("monthly_count", 0) or 0)
    monthly_reset = data.get("monthly_reset")

    # monthly_reset が無い/型が違う → 初期化
