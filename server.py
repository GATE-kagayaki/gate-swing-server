...
# ==================================================
# Cloud Tasks（失敗しても webhook を落とさない）
# ==================================================
def try_create_cloud_task(report_id: str, user_id: str, message_id: str) -> None:
    try:
        payload = json.dumps(
        ...
        print("Cloud Task created:", report_id)
    except Exception:
        print("Cloud Task create failed")
        print(traceback.format_exc())
...
def analyze_swing_with_mediapipe(video_path: str) -> Dict[str, Any]:
    ...
    with mp_pose.Pose(model_complexity=1) as pose:
        while cap.isOpened():
            ...
            min_hip = min(min_hip, angle(xy(LH), xy(RH), xy(LS)))
...
def handle_any(event: MessageEvent):
    ...
    if isinstance(msg, (VideoMessage, FileMessage)):
        ...
        try_create_cloud_task(report_id, user_id, msg.id)
        safe_line_reply(event.reply_token, make_initial_reply(report_id))
        return

    safe_line_reply(event.reply_token, "🎥 ゴルフスイング動画を送ってください。")
